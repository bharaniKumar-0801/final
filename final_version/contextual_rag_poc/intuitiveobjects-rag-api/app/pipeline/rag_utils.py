...
# from app.utils.llm import extract_keywords 
...
async def process_query(query: str, chat_id: str) -> str:
    ...
    try:
        ...
        # query = remove_think_tag(query)
        logger.info(f"[process_query] Cleaned query: {query}")  

        keyword_extractor = KeywordExtractor() 

        metadata_query_result = keyword_extractor.extract_keywords(query, top_n=10)

        

        # metadata_query_result = extract_keywords(query)

        logger.info(f"[process_query] Metadata query results: {metadata_query_result}")

        # metadata_query_result = await enrich_tokens(query)
        ...
        vector_results = similarity_search(query)
        ...
        def metadata_match(chunk, keywords):
            ...
            for kw in keywords:
                if str(kw).lower() in metadata_values:
                    logger.info(
                        f"Metadata match found: keyword '{kw}' in chunk metadata values '{metadata_values}' (chunk id: {chunk.get('id', chunk.get('chunk_id', 'N/A'))})"
                    )
                    return True
            return False

        def text_match(chunk, keywords):
            text_value = chunk.get("text", "").lower()
            for kw in keywords:
                if str(kw).lower() in text_value:
                    logger.info(
                        f"Text match found: keyword '{kw}' in chunk text (chunk id: {chunk.get('id', chunk.get('chunk_id', 'N/A'))})"
                    )
                    return True
            return False
        ...
        # If no metadata match, fall back to text match
        if not filtered_vector_results:
            logger.info("No metadata match found; checking text for keyword matches.")
            filtered_vector_results = [
                chunk for chunk in vector_results
                if text_match(chunk, metadata_query_result)
            ]
            if not filtered_vector_results:
                logger.info("No text match found; using all vector results as fallback.")
                filtered_vector_results = vector_results
            else:
                logger.info(f"Filtered vector results by text: {filtered_vector_results}")
        else:
            logger.info(f"Filtered vector results by metadata: {filtered_vector_results}")
...
# ...existing code...
        ...
        enriched_query = f"Answer the query using the context text and metadata.\n\nQuery:\n{query}\nContext:\n{ranked_chunks}"
...
async def run_bm25_keyword_search(query: str, top_n: int = 10) -> List[Dict]:
    ...
    # keyword_tokens = extract_keywords(query) 
    ...
    for filename in os.listdir(bm25_dir):
        ...
        try:
            ...
            top_results = bm25_index.get_top_n(metadata_query_result, n=top_n)
